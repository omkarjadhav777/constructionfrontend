...
const AddEntry = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('https://constructionbackend-j4rl.vercel.app/api/entries', entry);
  ...
  return (
    <Container maxWidth="md" sx={{ mt: 4 }}>
      <Paper elevation={3} sx={{ p: 3 }}>
        <Typography variant="h5" gutterBottom>🧱 Add Daily Entry</Typography>
        <form onSubmit={handleSubmit}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}>
              <TextField fullWidth name="blockAdded" label="Block Added" type="number" value={entry.blockAdded} onChange={handleChange} />
            </Grid>
            <Grid item xs={12} sm={6}>
            ...
            {/* Block Sold to Buyers */}
            <Grid item xs={12}>
              <Typography variant="subtitle1">Block Sold to Buyers</Typography>
              ...
              <Button variant="outlined" startIcon={<AddCircle />} onClick={addBuyer}>Add Buyer</Button>
            </Grid>

            <Grid item xs={12}>
              <Button variant="contained" color="primary" fullWidth type="submit">Submit Entry</Button>
            </Grid>
          </Grid>
        </form>
      </Paper>
